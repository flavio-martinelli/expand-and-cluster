"""
 ...
 #
 # Description: description of Lottery Ticket Hypothesis experiment (it is also the most generic Desc class!)
 #
"""
...
class LotteryDesc(Desc):
    ...
    def create_from_args(cls, args: argparse.Namespace) -> 'LotteryDesc':
        ...
        extraction_hparams = hparams.ExtractionHparams(gamma=None, beta=None)  # extraction hparams are not used here.

        # Create the desc.
        desc = cls(model_hparams, dataset_hparams, training_hparams, pruning_hparams, extraction_hparams)
    ...
    def run_path(self, global_seed: int, pruning_level: Union[str, int], experiment: str = 'main'):
        """The location where any run is stored."""

        if not isinstance(global_seed, int) or global_seed < -1:
            raise ValueError('Bad seed: {}'.format(global_seed))

        return os.path.join(get_platform().root, self.hashname,
                            f'seed_{global_seed}', f'level_{pruning_level}', experiment)
