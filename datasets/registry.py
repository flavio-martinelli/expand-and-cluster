"""
 ...
 #
 # Description: registry of Datasets -- initializes the selected dataset
 #
"""
...
def iterations_per_epoch(dataset_hparams: DatasetHparams):
    ...
    if dataset_hparams.dataset_name in registered_datasets:
        if dataset_hparams.dataset_name == "teacher":
            num_train_examples = dataset_hparams.samples
        else:
...
def num_classes(dataset_hparams: DatasetHparams):
    ...
    if dataset_hparams.dataset_name in registered_datasets:
        if dataset_hparams.dataset_name == "teacher":
            dataset = registered_datasets[dataset_hparams.dataset_name].Dataset(dataset_hparams, use_augmentation=False)
            num_classes = dataset.num_classes()  # TODO: there must be a cleaner way to do this
        else:
